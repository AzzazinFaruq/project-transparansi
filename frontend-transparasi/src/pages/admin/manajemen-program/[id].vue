      ...
      <v-row>
        <v-col cols="12" md="8">
          <v-form v-if="user.status == 'Menunggu' || user.status == 'Dalam Proses'">
            ...
            <!-- Tombol Aksi -->
            <div class="d-flex gap-2 mt-5" v-if="user.status=='Menunggu'">
              <v-btn
                color="#387144"
                style="color: white"
            ...
            <div class="d-flex gap-2 mt-5" v-if="user.status=='Dalam Proses'">
              <v-btn
                color="#387144"
                style="color: white"
...
export default{
  ...
  .catch(error => {
    ...
    async accept() {
      try {
        // Konfirmasi terima program
        const result = await Swal.fire({
          title: 'Terima Program?',
          text: "Program akan diproses setelah diterima",
          icon: 'question',
        ...
        if (result.isConfirmed) {
          ...
          // Proses terima program
          await axios.get(`/api/program/accept/${this.$route.params.id}`);
          
          // Tampilkan sukses
          await Swal.fire({
            icon: 'success',
            title: 'Program Diterima',
            text: 'Program telah berhasil diterima dan akan diproses',
        ...
        Swal.fire({
          icon: 'error',
          title: 'Oops...',
          text: 'Terjadi kesalahan saat menerima program',
